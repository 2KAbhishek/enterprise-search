...
enterprise-search is a `<utility/tool>` that allows `<target_audience>` to `<action>`.
...
Installing enterprise-search is as simple as cloning and symlinking!
...
```bash
USAGE:
    enterprise-search [FLAGS] [OPTIONS]
Example:
    enterprise-search
```
...
enterprise-search was inspired by `<reason/idea>`.
