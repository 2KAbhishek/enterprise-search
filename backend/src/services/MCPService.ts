...
export class MCPService {
    ...
    private loadConfig(): void {
        try {
            ...
            console.log(
                `✅ Loaded MCP config with ${this.config.servers.length} servers`
            );
        } catch (error) {
    ...
    private async connectToServerWithRetry(
        ...
        try {
            ...
            console.error(
                `Failed to connect to MCP server ${config.name} (attempt ${attemptCount + 1}):`,
                error
            );
            ...
            if (attemptCount < this.maxRetries) {
                ...
                console.log(
                    `Retrying connection to ${config.name} in ${this.retryDelay}ms...`
                );
                ...
                console.error(
                    `Max retry attempts (${this.maxRetries}) reached for ${config.name}. Giving up.`
                );
                this.retryAttempts.set(config.name, 0);
            }
        }
        ...
        for (const [serverName, client] of this.clients) {
            try {
                ...
                try {
                    ...
                    console.warn(
                        `${serverName} tools not available:`,
                        toolError
                    );
                }
                ...
                try {
                    ...
                    console.warn(
                        `${serverName} resources not available:`,
                        resourceError
                    );
                }
            } catch (error) {
    ...
    private async getServerCapabilities(
        ...
        try {
            ...
            return capabilities.length > 0
                ? capabilities.join(', ')
                : 'Connected (capabilities unknown)';
        } catch (error) {
